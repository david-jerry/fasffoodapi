...
from .models import Locations, MenuItems
...
class Cart:
    ...
    #     products = MenuItems.objects.filter(unique_id__in=product_ids)
    #     cart = self.cart.copy()
    #     LOGGER.info(cart.values())
    #     for product in products:
    #         p = MenuItemsSerializer(product, context={'request':self.request}).data
    #         LOGGER.info(p)
    #         cart[str(product.unique_id)]["product"] = p
    #     for item in cart.values():
    #         location = Locations.objects.get(id=int(item['pickup_location']))

    #         item['pickup_location'] = LocationsSerializer(location, many=False, context={'request':self.request}).data
